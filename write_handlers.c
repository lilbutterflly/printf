...
int write_num(int ind, char buffer[], int flags, int width, int prec, int length, char padd, char extra_c)
{
	...
	if (width > length)
	{
		for (i = 1; i < width - length + 1; i++)
			buffer[i] = padd;
		buffer[i] = '\0';
		if (falgs & F_MINUS && padd == '')/* Asign extra char to left of buffer*/
		{
			if (extra_c)
				buffer[--ind] = extra_c;
...
int write_pointer(char buffer[], int ind, int length, int width, int flags, char padd, char extra_c, int padd_start)
{
	...
	if (width > length)
	{
		for (i = 3; i < width - length + 3; i++)
			buffer[i] = padd;
		buffer[i] = '\0';
		if (flags & F_MINUS && padd == '') /* Asign wxtra char to left of buffer*/
		{
			buffer[--ind] = 'x';
			buffer[--ind] = '0';
			if (extra_c)
		...
		else if (!(flags & F_MINUS) && padd == '0') /*extra char to ledt of padd*/
		{
			if (extra_c)
				buffer[--padd_start] = extra_c;
			buffer[1] = '0';
